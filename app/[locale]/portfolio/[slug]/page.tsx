...
import { ArrowLeft, Github, ExternalLink, Code2, Star, GitFork, ChevronRight, CheckCircle2 } from "lucide-react";
          ...
          <div className="lg:col-span-7 space-y-16">
            ...
            {project.slug === "mft-algo-bot" && (
              <div className="space-y-6">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Live Market Data</p>
                <LiveMarketData />
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="p-8 rounded-[2.5rem] bg-white/[0.03] border border-white/10 backdrop-blur-sm space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Timeline</p>
                  <p className="text-2xl font-bold">{project.date}</p>
               </div>
               <div className="p-8 rounded-[2.5rem] bg-white/[0.03] border border-white/10 backdrop-blur-sm space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Role</p>
            ...
            <div className="space-y-8">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Technology Stack</p>
                <div className="flex flex-wrap gap-3">
                   {project.technologies.map(tech => (
                       <span key={tech} className="px-6 py-3 bg-white/[0.03] hover:bg-white/[0.08] transition-colors rounded-2xl text-xs font-black uppercase tracking-widest border border-white/10">
                           {tech}
                       </span>
                   ))}
                </div>
                ...
                {project.liveUrl && project.liveUrl !== "#" && (
                  <Magnetic>
                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">
                        <Button size="lg" className="rounded-full px-12 h-20 text-sm font-black uppercase tracking-[0.2em] gap-3 bg-white text-black hover:bg-slate-200">
                ...
                {project.githubUrl && (
                  <Magnetic>
                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer">
                        <Button variant="outline" size="lg" className="rounded-full px-12 h-20 text-sm font-black uppercase tracking-[0.2em] gap-3 border-white/10 bg-white/5 hover:bg-white/10">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            <div className="relative aspect-square md:aspect-[4/5] rounded-[4rem] overflow-hidden bg-slate-900 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-white/10 group">
              {project.src.startsWith("bg-") ? (
                <div className={`w-full h-full ${project.src} flex flex-col items-center justify-center text-primary/20 group-hover:scale-110 transition-transform duration-[2s]`}>
                  <Code2 size={160} strokeWidth={0.5} />
                  <span className="mt-8 text-2xl font-black tracking-widest text-white/5 uppercase italic">Engineered Preview</span>
                </div>
              ) : (
                <Image
                  src={project.src}
                ...
                <div className="absolute bottom-12 left-12 flex gap-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Stars</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Forks</span>
