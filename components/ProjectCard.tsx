      ...
      <div className="relative aspect-[16/10] overflow-hidden">
        {imageSrc && (imageSrc.startsWith('/') || imageSrc.startsWith('http')) ? (
          <Image
            src={imageSrc}
            alt={
              project.titleKey ? t(`projects.${project.titleKey}`) : project.title || project.slug
            }
            fill
          ...
          <div
            className={`w-full h-full ${imageSrc} flex items-center justify-center text-primary/20 bg-slate-900`}
          >
      ...
      <div className="p-8 flex flex-col h-full">
        <div className="flex flex-wrap gap-2 mb-4">
          {project.technologies?.slice(0, 3).map((tech) => (
            <span
              key={tech}
              className="text-[10px] font-bold px-3 py-1 bg-white/5 text-slate-300 border border-white/10 rounded-full uppercase tracking-wider"
            >
              {tech}
            </span>
          ))}
        ...
        <Link
          ...
          className="group/title flex items-start justify-between"
        >
          ...
          <ArrowUpRight
            className="text-white/40 group-hover/title:text-primary group-hover/title:translate-x-1 group-hover/title:-translate-y-1 transition-all"
            size={20}
          />
        </Link>
        ...
        <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/5">
          ...
          <Link
            href={`/portfolio/${project.slug}`}
            className="group/btn flex items-center gap-2 text-xs font-bold text-primary uppercase tracking-widest"
          >
            Learn More
