  ...
  return (
    <div className="container mx-auto px-4 md:px-8 py-32 md:py-48">
      <div className="text-center mb-32">
        ...
        >
          Choose the perfect package for your story. All packages include high-end retouching and private online gallery.
        </motion.p>
      </div>
        ...
        {packages.map((pkg) => (
          ...
          >
            ...
            <div className="mb-12 flex justify-between items-start">
              <div className={cn("w-20 h-20 rounded-[1.5rem] flex items-center justify-center transition-transform group-hover:scale-110 duration-500", 
                pkg.popular ? "bg-white/10" : "bg-primary/5 text-primary")}>
                <pkg.icon size={36} />
              </div>
              <div className="text-right">
                <p className={cn("text-5xl font-black tracking-tighter mb-1", pkg.popular ? "text-white" : "text-foreground")}>
                  {pkg.price}
                </p>
                <p className={cn("text-[10px] font-black uppercase tracking-[0.2em]", pkg.popular ? "text-white/40" : "text-muted-foreground/60")}>
                  Full Experience
                </p>
              </div>
            </div>
            ...
            <div className="mb-10">
              ...
              <p className={cn("text-lg font-medium leading-relaxed opacity-70", pkg.popular ? "text-white" : "text-muted-foreground")}>
                {pkg.description}
              </p>
            </div>

            <div className={cn("h-px w-full mb-10", pkg.popular ? "bg-white/10" : "bg-border/50")} />

            <ul className="space-y-6 mb-16 flex-1">
              {pkg.features.map((feature, i) => (
                <li key={i} className="flex items-start gap-5 text-base font-semibold">
                  <div className={cn("mt-1 flex-shrink-0", pkg.popular ? "text-white" : "text-primary")}>
